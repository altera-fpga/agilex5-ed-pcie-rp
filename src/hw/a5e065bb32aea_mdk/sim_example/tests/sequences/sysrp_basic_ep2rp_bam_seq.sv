class sysrp_basic_ep2rp_bam_seq extends sysrp_base_seq;
  rand int sequence_length ;

  `uvm_object_utils(sysrp_basic_ep2rp_bam_seq);

  /** Declare a typed sequencer object that the sequence can access */
  `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

  function new (string name = "sysrp_basic_ep2rp_bam_seq");
    super.new(name);
  endfunction : new

  virtual function void build_phase(uvm_phase phase);
   `uvm_info ("build_phase", "[sysrp_basic_ep2rp_bam_seq] Entered Sequence Build Phase...",UVM_LOW);
   `uvm_info ("build_phase", "[sysrp_basic_ep2rp_bam_seq] Exiting Sequence Build Phase...",UVM_LOW)
  ...
  task body();
    ...
    `uvm_do_on_with(mem_request_seq,p_sequencer.endpoint_vseqr.endpoint_virt_seqr.driver_transaction_seqr[0],{ mem_request_seq.transaction_type == svt_pcie_driver_app_transaction::MEM_WR;
                                                                                  mem_request_seq.address[31:12] dist {[20'h8_0000 : 20'hB_0000]:= 1,[20'hB_0000 : 20'hF_0000]:= 1};
