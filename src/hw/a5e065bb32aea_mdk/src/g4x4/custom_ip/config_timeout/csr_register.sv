...
module csr_register #(
    ...
    input logic [RESP_WIDTH-1:0]       cs_snoop_resp_i,         //  resp signal from p tile ip to set error status reg value[0]
     ...
     input logic bas_timeout,
     input logic bas_ur_ca_error,
    ...
    localparam	TIMEOUT_REG = 5'b00100; //04                   
	 ...
	 localparam BAS_ERROR_STAT = 5'h14;

    // default parameters 	 
	 localparam	TIMEOUT_DEFAULT  = 32'd1000000;              // recommended: 10ms. based on clock 100MHz, 
                                                            // number of clock cycles required 10ms/1ns=1,000,000
	 localparam CLOCK_FRQ_DEFAULT= 32'h7a120; 
	 localparam SOFT_RST_DEFAULT = 30'h0;
	 ...
	 logic[29:0]          soft_reset_scratch_reg;           // register to store soft reset scratch
     logic axi_st_soft_reset_reg, axi_lite_soft_reset_reg;  // register to store soft reset axi-st, axi-lite rst
	 logic[REG_WIDTH-1:0] error_status_reg, bas_error_status_reg; // register to store error
...
assign		hip_status_reg[0]   = hip_status_linkup;
   ...
   always @ (posedge clk_i) begin
	 if (~rstn_i) begin
		...
		axi_lite_soft_reset_reg  <= '0;
		axi_st_soft_reset_reg    <= '0;
		soft_reset_scratch_reg <= SOFT_RST_DEFAULT;
		error_status_reg <= ERROR_STAT_DEFAULT;
		bas_error_status_reg <= ERROR_STAT_DEFAULT;
	 end else begin
       if (csr_write_i & csr_address_i == TIMEOUT_REG) begin
    	...
    	soft_reset_scratch_reg  <= csr_writedata_i[31:2];	 	
    	axi_lite_soft_reset_reg <= csr_writedata_i[1];	 	
    	axi_st_soft_reset_reg   <= csr_writedata_i[0];	 	
	   end else if (csr_write_i & csr_address_i == ERROR_STAT) begin
    	error_status_reg <= csr_writedata_i;
	   end else if (csr_write_i & csr_address_i == BAS_ERROR_STAT) begin
    	bas_error_status_reg <= csr_writedata_i;
	   end else if (timeout) begin
    	error_status_reg[1] <= '1;
	   end else if (non_zero_resp) begin
    	error_status_reg[0] <= '1;	
	   end else if (bas_timeout) begin
    	bas_error_status_reg[1] <= '1;
	   end else if (bas_ur_ca_error) begin
    	bas_error_status_reg[0] <= '1;	
       end 
     end
   end

  logic axi_st_start, axi_lite_start, unused_axi_st_overf, unused_axi_lt_overf;
  logic [11:0] axi_st_rst_cnt, axi_lite_rst_cnt;
  ...
  always @ (posedge clk_i) begin
    if (~rstn_i) begin
      ...
      axi_st_rst_cnt <= '0;
      axi_lite_rst_cnt <= '0;
		unused_axi_st_overf <= '0;
		unused_axi_lt_overf <= '0;
    end else begin
      // csr_axi_st_rst
      if (axi_st_soft_reset_reg & !axi_st_start) begin
        csr_axi_st_rst <= '1;
        axi_st_start <= '1;
      end else if (axi_st_start & axi_st_rst_cnt[11]) begin
        csr_axi_st_rst <= '0;
        axi_st_start <= '0;    
      end

      // csr_axi_lite_rst
      if (axi_lite_soft_reset_reg & !axi_lite_start) begin
        csr_axi_lite_rst <= '1;
        axi_lite_start <= '1;
      end else if (axi_lite_start & axi_lite_rst_cnt[11]) begin
        csr_axi_lite_rst <= '0;
        axi_lite_start <= '0;    
      end
      ...
      // axi_st_rst_cnt
      if (axi_st_start)
        {unused_axi_st_overf, axi_st_rst_cnt} <= axi_st_rst_cnt + 'h1;
      else 
        axi_st_rst_cnt <= '0;
	  
      // axi_lite_rst_cnt
      if (axi_lite_start)
        {unused_axi_lt_overf, axi_lite_rst_cnt} <= axi_lite_rst_cnt + 'h1;
      else 
        axi_lite_rst_cnt <= '0;
...
always @ (posedge clk_i) begin
    ...
    case(csr_address_i)
	    ...
	    SOFT_RESET : begin 
          csr_readdata_o[31:2] <= soft_reset_scratch_reg;
          csr_readdata_o[1]    <= csr_axi_lite_rst;
          csr_readdata_o[0]    <= csr_axi_st_rst;
        end
		 ERROR_STAT : csr_readdata_o <= error_status_reg;
		 BAS_ERROR_STAT : csr_readdata_o <= bas_error_status_reg;
