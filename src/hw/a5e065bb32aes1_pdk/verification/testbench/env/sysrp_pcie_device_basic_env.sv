...
function void sysrp_pcie_device_basic_env::build_phase( uvm_phase phase );
   ...
   this.pcie_cfg.endpoint_cfg.model_instance_scope = "sysrp_top_tb.endpoint0";
      ...
      if(status1)begin
         ...
         this.pcie_cfg.endpoint_cfg.pcie_cfg.tl_cfg.remote_max_payload_size      = RP_MAX_payload; // 128DW - Specifies the Max payload size (in bytes) to expect from the device at the other end of the link. Has to be set as the same value as the DUT's max_payload_size
      end
      else begin
         `uvm_info("body", $sformatf("ENV: rp_max_payload_size is set to default(512)"), UVM_LOW);
         this.pcie_cfg.endpoint_cfg.pcie_cfg.tl_cfg.remote_max_payload_size      = 512; // 128DW - Specifies the Max payload size (in bytes) to expect from the device at the other end of the link. Has to be set as the same value as the DUT's max_payload_size
      end
      ...
      if(status3) begin
        `uvm_info("body", $sformatf("ENV: ep_max_payload_size = %d", EP_MAX_payload), UVM_LOW);
         this.pcie_cfg.endpoint_cfg.pcie_cfg.dl_cfg.max_payload_size             = EP_MAX_payload;  //Maximum of 64DW payload can be sent from Endpoint
      end
      else begin
        `uvm_info("body", $sformatf("ENV: ep_max_payload_size is set to default(512)"), UVM_LOW);
         this.pcie_cfg.endpoint_cfg.pcie_cfg.dl_cfg.max_payload_size             = 512;  //Maximum of 64DW payload can be sent from Endpoint
      end
      ...
      if(status4)begin
         ...
         this.pcie_cfg.endpoint_cfg.target_cfg[0].min_mem_cpl_latency_ns = MIN_CPL_LATENCY; // Minimum delay for the completions from PCIE VIP
      end
      ...
      if(status5)begin
         ...
         this.pcie_cfg.endpoint_cfg.target_cfg[0].max_mem_cpl_latency_ns = MAX_CPL_LATENCY; // Maximum delay for the completions from PCIE VIP
      end
      ...
      // Below was added because unaligned read request from HPS was reaching
      // Endpoint with first byte enable as 'hF even though address was not
      // alligned. Ideally first byte enable should go as 'h0. But since AVMM protocol 
      // supports only alligned transfer, read goes as full byteenable. There is no read 
      // strb signal to have partial read data(In write of axi, wstrb is there to do partial writes).
      // So this has to be taken care in BFM.
...
function void sysrp_pcie_device_basic_env::AXIS_HOST_CFG();
 ...
 function void sysrp_pcie_device_basic_env::H2F_AXI4_CFG();
	  ...
	  h2f_axi_cfg.slave_cfg[0].addr_width = 32;
      `endif
      h2f_axi_cfg.slave_cfg[0].default_wready = 1; //ST changed from 0 to 1
      h2f_axi_cfg.master_cfg[0].default_rready = 1;

      h2f_axi_cfg.wready_watchdog_timeout = 0; // Added for transactions from BAS side as length > 'h8 is taking some more time
      h2f_axi_cfg.rdata_watchdog_timeout   = 0; //   UVM_ERROR: uvm_test_top.env.axi_h2f_system_env.master[0] [track_timeout] {OBJECT_NUM('d0) PORT_ID('d0) PORT_NAME() TYPE(READ) COHERENT_XACT_TYPE(DVMMESSAGE) ID('h1) SECURE('d0) ADDR('ha0000004) CACHE_TYPE('d3)  START_TIME(205247000000.00 ps)} Timed out waiting for rvalid. Timeout = 'd256000 clock cycles('f1024000.000000 time)
