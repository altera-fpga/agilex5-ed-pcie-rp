...
class sysrp_scoreboard extends uvm_scoreboard;
   ...
   function void build_phase(uvm_phase phase);
	...
	bit[31:0]  byte_enable_data_at_pcie, out_data_at_pcie;
        ...
        if(trans.tlp.tlp_type == 5'h0 && ($size(trans.tlp.payload)!=0)) begin //{ 
           `uvm_info(get_type_name(),$sformatf("ST2 write_pcie_port_tx: Write Pkt transmitted by PCIe VIP \n %s",trans.sprint()),UVM_LOW)
	   pcie_tx_wr_count++;
	   `uvm_info(get_type_name(),$sformatf("ST2 write_pcie_port_tx: pcie_tx_wr_count=%0d",pcie_tx_wr_count),UVM_LOW)
           payload_size = $size(trans.tlp.payload);
           `uvm_info(get_type_name(),$sformatf("ST2 write_pcie_port_tx: PAYLOAD SIZE=%0h\n",payload_size),UVM_LOW)
	   pcie_tx_write_fifo_q.push_back(trans);
           ...
           for(int i=0;i<payload_size;i++) begin //{
	         ...
	         for(int k=0;k<=3;k++)  begin //{
                 ...
                 `uvm_info(get_type_name(),$sformatf("ST2 write_pcie_port_tx: DATA sent by PCIE VIP=%0h\n",byte_enable_data_at_pcie),UVM_LOW)
              ...
              `uvm_info(get_type_name(),$sformatf("ST2 write_pcie_port_tx: PCIe Address-Data Associative Array [%0h] = %0h",address_pcie_write_trans,pcie_address_wdata_assoc_array[address_pcie_write_trans]),UVM_LOW);
        end //}
	//2. Check pcie MRd
	else if(trans.tlp.tlp_type == 5'h0 && ($size(trans.tlp.payload)==0))  begin  //{
           `uvm_info(get_type_name(),$sformatf("ST2 write_pcie_port_tx: Read Pkt transmitted by PCIe VIP \n %s",trans.sprint()),UVM_LOW)
	   pcie_tx_readpkt_q.push_back(trans);
	   pcie_tx_read_fifo_q.push_back(trans);
	   pcie_tx_rd_count++;
	   `uvm_info(get_type_name(),$sformatf("ST2 write_pcie_port_tx: pcie_tx_rd_count=%0d",pcie_tx_rd_count),UVM_LOW)
      end //}
   ...
   virtual function void write_pcie_port_rx_from_slave(svt_pcie_dl_tlp_monitor_transaction trans);
      svt_pcie_dl_tlp_monitor_transaction   pcie_tx_rd_trans,pcie_rx_rd_trans;
      bit[63:0]  address_pcie_read_trans;
      int        payload_size;
      bit[9:0]   length_pcie_read_trans;
      int        dwords_received_from_slave;


      if((trans.tlp.tlp_type == svt_pcie_tlp::CPL) && (trans.tlp.fmt == svt_pcie_tlp::WITH_DATA_3_DWORD)) begin //{
         pcie_rx_readpkt_q.push_back(trans);
         `uvm_info(get_type_name(),$sformatf(" SCB:: Pkt received by PCIe VIP from AXI Slave\n %s", trans.sprint()),UVM_LOW)
         pcie_rx_slavecpl_count++;

	 if((has_pcie_tx==1) && (pcie_tx_read_fifo_q.size() > 0) && (!unaligned_enb)) begin //{
	    dwords_received_from_slave = 0;
            pcie_tx_rd_trans = pcie_tx_read_fifo_q.pop_front();
            address_pcie_read_trans = pcie_tx_rd_trans.tlp.address;
            length_pcie_read_trans  = pcie_tx_rd_trans.tlp.length;

            payload_size = $size(trans.tlp.payload);
	    for(int i=0;i<payload_size;i++) begin //{
               `uvm_info(get_type_name(),$sformatf(" SCB:: DATA received at PCIE VIP from slave=%0h\n",trans.tlp.payload[i]),UVM_LOW)
                pcie_address_rdata_assoc_array[address_pcie_read_trans] = {pcie_address_rdata_assoc_array[address_pcie_read_trans],trans.tlp.payload[i]};
                `uvm_info(get_type_name(),$sformatf("Address `h%0h Array = %0h ",address_pcie_read_trans,pcie_address_rdata_assoc_array[address_pcie_read_trans]),UVM_LOW);
                dwords_received_from_slave++;
            end  //}

	    if(dwords_received_from_slave == length_pcie_read_trans)  
	      `uvm_info(get_type_name(),$sformatf("ST2:dwords_received_from_slave=%0d, length_pcie_read_trans=%0d", dwords_received_from_slave, length_pcie_read_trans),UVM_LOW)
	      
           if(pcie_address_rdata_assoc_array[address_pcie_read_trans] == pcie_address_wdata_assoc_array[address_pcie_read_trans]) begin
              `uvm_info(get_type_name(),$sformatf("[compare_data_coming_from_slave] : DATA MATCHED for address `h%0h Write Payload = %0h      Read Payload = %0h",address_pcie_read_trans,pcie_address_wdata_assoc_array[address_pcie_read_trans],pcie_address_rdata_assoc_array[address_pcie_read_trans]),UVM_LOW);
           end
           else begin
              `uvm_error(get_type_name(), $sformatf("[compare_data_coming_from_slave] : DATA MISMATCHED for address `h%0h Write Payload = %0h      Read Payload = %0h,",address_pcie_read_trans,pcie_address_wdata_assoc_array[address_pcie_read_trans],pcie_address_rdata_assoc_array[address_pcie_read_trans]));
           end
        end //}
      end  //}
   ...
   virtual function void write_pcie_port_rx(svt_pcie_dl_tlp_monitor_transaction trans);
      ...
      pcie_rx_wr_count++;
      `uvm_info(get_type_name(),$sformatf("ST2b write_pcie_port_rx: pcie_rx_wr_count=%0d\n",pcie_rx_wr_count),UVM_LOW)
         ...
         for(int i=0;i<payload_size;i++) begin //{
		  ...
		  j = 0;
		     ...
		     if(axi_strobe_tx > 0) j = 8 ;
	          ...
	          `uvm_info(get_type_name(),$sformatf("ST2a write_pcie_port_rx: Exp First_be=0x%0h, Act First_be =0x%0h", axi_strobe_tx, l_first_last_be),UVM_LOW)

	          if(l_first_last_be !==  axi_strobe_tx)
	              `uvm_error(get_type_name(), $sformatf("ST2 write_pcie_port_rx: Mismatch Exp First_be=0x%0h, Act First_be =0x%0h", axi_strobe_tx, l_first_last_be));
	       end //}

	       //
	       if(l_first_last_be != 0) begin //{
                  `uvm_info(get_type_name(),$sformatf("ST2 write_pcie_port_rx: MWr received at PCIE Endpoint VIP=%0h\n",trans.tlp.payload[i]),UVM_LOW)
	 ...
	 if(!error_case) compare_data_from_axi();
      end  //}
   ...
   virtual function void write_axi_port_rx(svt_axi_transaction trans);
	 ...
	 if(l_address_pcie_read_trans[63:0] !== axi_rx_pkt_address[63:0]) 
            `uvm_error(get_type_name(), $sformatf("ST2 write_axi_port_rx: Mismatch Exp address_pcie_read_trans=0x%0h, Act axi_rx_pkt_address =0x%0h", l_address_pcie_read_trans, axi_rx_pkt_address));
	 ...
	 l_address_pcie_write_trans = pcie_txwrite_trans.tlp.address;
         ...
         if( (!unaligned_enb) && (l_address_pcie_write_trans[63:0] !== axi_rx_pkt_address[63:0]) ) 
            `uvm_error(get_type_name(), $sformatf("ST2 write_axi_port_rx: Mismatch Exp address_pcie_write_trans=0x%0h, Act axi_rx_pkt_address =0x%0h", l_address_pcie_write_trans, axi_rx_pkt_address));
	    ...
	    for(int j=0;j<16;j++) begin // {
	       ...
	       if(strobe == 'hF) begin //{
                  ...
                  `uvm_info(get_type_name(),$sformatf("ST2 write_axi_port_rx: I=%0d J=%0d ADDR=%0h DATA received at SLAVE=%0h\n",i,j,axi_rx_pkt_address,out_BE),UVM_LOW)
	 	  ...
	 	  for(int k=0;k<=3;k++) begin  //{
                  ...
                  `uvm_info(get_type_name(),$sformatf("ST2a write_axi_port_rx: I=%0d J=%0d ADDR=%0h DATA received at SLAVE=%0h\n",i,j,axi_rx_pkt_address,byte_enable_data_at_slave),UVM_LOW)
       ...
       check_pcie_counter++;
       `uvm_info(get_type_name(),$sformatf("ST2 write_axi_port_rx: QUEUE TX [Transmitted from PCIe] = %p",pcie_payload_tx_q),UVM_LOW);
       `uvm_info(get_type_name(),$sformatf("ST2 write_axi_port_rx: QUEUE RX [Received at AXI Slave] = %p",axi_payload_rx_q),UVM_LOW);
   ...
   virtual function void write_axi_port_tx(svt_axi_transaction trans);
      ...
      if((trans.addr > 'h1000_0000) && (trans.xact_type == 'h1) && ((trans.addr[31:16] !== 'h2008)))  begin //{
         `uvm_info(get_type_name(),$sformatf("ST2 write_axi_port_tx:  Write Pkt sent by AXI Master \n %s",trans.sprint()),UVM_LOW)
 	 axi_tx_wr_count++;
   ...
   virtual function void write_axi_port_tx_from_slave(svt_axi_transaction trans);
      svt_axi_transaction   axi_tx_rd_trans;
      bit[63:0]   address_axi_read_trans;
      int         data_array_size;
      bit[31:0]   out,out_BE,byte_enable_data_at_slave;


      `uvm_info(get_type_name(),$sformatf(" SCB:: Pkt transmitted by AXI Slave \n %s",trans.sprint()),UVM_LOW)
      axi_tx_slave_count++;
      
      	if( (has_axi_tx==1) && (axi_tx_read_fifo_q.size() > 0) && (!unaligned_enb) ) begin //{
           axi_tx_rd_trans = axi_tx_read_fifo_q.pop_front();
           address_axi_read_trans = axi_tx_rd_trans.addr;
           data_array_size = $size(axi_tx_rd_trans.data);
	   for(int i=0;i<data_array_size;i++) begin //{
	      for(int j=0;j<4;j++) begin  //{
		 if(axi_tx_rd_trans.data[i][32*j +: 32] != 'h0) begin //{
                    out    = axi_tx_rd_trans.data[i][32*j +: 32];
                    out_BE = {out[7:0], out[15:8], out[23:16], out[31:24]};
                    `uvm_info(get_type_name(),$sformatf(" SCB:: I=%0d J=%0d ADDR=%0h DATA received at Master for Memory Read=%0h\n",i,j,address_axi_read_trans,out_BE),UVM_LOW)
                    axi_address_rdata_assoc_array[address_axi_read_trans] = {axi_address_rdata_assoc_array[address_axi_read_trans],out_BE};
                 end //}
              end //}
           end //}

           if(axi_address_rdata_assoc_array[address_axi_read_trans] == axi_address_wdata_assoc_array[address_axi_read_trans])
               begin
                  `uvm_info(get_type_name(),$sformatf("[compare_data_coming_from_endpoint] : DATA MATCHED for address `h%0h Write Payload = %0h Read Payload = %0h",address_axi_read_trans,axi_address_wdata_assoc_array[address_axi_read_trans],axi_address_rdata_assoc_array[address_axi_read_trans]),UVM_LOW);
               end
             else
               begin
                 `uvm_error(get_type_name(), $sformatf("[compare_data_coming_from_endpoint] : DATA MISMATCHED for address `h%0h Write Payload = %0h Read Payload = %0h,",address_axi_read_trans,axi_address_wdata_assoc_array[address_axi_read_trans],axi_address_rdata_assoc_array[address_axi_read_trans]));
               end
         end //}
   ...
   function void report_phase(uvm_phase phase);
      ...
      `uvm_info(get_type_name(), $sformatf("ST1 INFO: pcie_tx_wr_count=%0d, pcie_tx_rd_count=%0d", pcie_tx_wr_count, pcie_tx_rd_count), UVM_LOW)
      `uvm_info(get_type_name(), $sformatf("ST1 INFO: pcie_rx_wr_count=%0d, pcie_rx_slavecpl_count=%0d", pcie_rx_wr_count, pcie_rx_slavecpl_count), UVM_LOW)
      `uvm_info(get_type_name(), $sformatf("ST1 INFO: axi_rx_wr_count=%0d, axi_rx_rd_count=%0d", axi_rx_wr_count, axi_rx_rd_count), UVM_LOW)
      `uvm_info(get_type_name(), $sformatf("ST1 INFO: axi_tx_wr_count=%0d, axi_tx_rd_count=%0d", axi_tx_wr_count, axi_tx_rd_count), UVM_LOW)
      `uvm_info(get_type_name(), $sformatf("ST1 INFO: axi_tx_slave_count=%0d", axi_tx_slave_count), UVM_LOW)
