/**
 * Abstract:  sysrp_bas_cplstat_ca_test
...
///--- class ctl_cplca_pcie_target_callback  ---
class ctl_cplca_pcie_target_callback extends svt_pcie_target_app_callback;
   svt_pcie_tlp_exception my_tlp_exc = new("my_tlp_exc");

   `uvm_object_utils(ctl_cplca_pcie_target_callback)

   function new(string name ="ctl_cplca_pcie_target_callback");
      super.new(name);
   endfunction
...
endclass: ctl_cplca_pcie_target_callback


///---class sysrp_bas_cplstat_ca_test ---
class sysrp_bas_cplstat_ca_test extends sysrp_base_test;
   int pcie_tx_scbd;
   int axi_tx_scbd;

   /** UVM Component Utility macro */
   `uvm_component_utils(sysrp_bas_cplstat_ca_test)

   ctl_cplca_pcie_target_callback ctl_cpl_ep_packet;

   /** Class Constructor */
   function new(string name = "sysrp_bas_cplstat_ca_test", uvm_component parent=null);
  ...
  virtual function void build_phase(uvm_phase phase);
     ...
     `uvm_info ("build_phase", "[sysrp_bas_cplstat_ca_test] Entered Build Phase...",UVM_LOW);
     ...
     `uvm_info ("build_phase", "[sysrp_bas_cplstat_ca_test] Exiting Build Phase...",UVM_LOW)
