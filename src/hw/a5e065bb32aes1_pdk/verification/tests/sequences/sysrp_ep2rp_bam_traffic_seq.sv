class sysrp_ep2rp_bam_traffic_seq extends sysrp_base_seq;
  
  //rand int no_of_transactions ;
  rand int sequence_length ;

  `uvm_object_utils(sysrp_ep2rp_bam_traffic_seq);

  /** Declare a typed sequencer object that the sequence can access */
  `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

  function new (string name = "sysrp_ep2rp_bam_traffic_seq");
    super.new(name);
  endfunction : new

  virtual function void build_phase(uvm_phase phase);
   `uvm_info ("build_phase", "[sysrp_ep2rp_bam_traffic_seq] Entered Sequence Build Phase...",UVM_LOW);
   `uvm_info ("build_phase", "[sysrp_ep2rp_bam_traffic_seq] Exiting Sequence Build Phase...",UVM_LOW)
  ...
  task body();
    ...
    // Addr = 'hFBA0_0100
    //===============================
    `uvm_info(get_name(), "MEM_WR on 'hFBA0_0100 from Endpoint to RP", UVM_LOW)
     ep_mem_wr_req (.addr_('hFBA0_0100), .length_(16));
    `uvm_info(get_name(), "MEM_RD on 'hFBA0_0100 from Endpoint to RP", UVM_LOW)
     ep_mem_rd_req (.addr_('hFBA0_0100), .length_(16));
