class sysrp_ep2rp_bam_mixsizepkts_perf_seq extends sysrp_base_seq;
  
  //rand int no_of_transactions ;
  rand int sequence_length ;
  rand bit sel0for2G_sel1for6G;

  `uvm_object_utils(sysrp_ep2rp_bam_mixsizepkts_perf_seq);

  /** Declare a typed sequencer object that the sequence can access */
  `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

  function new (string name = "sysrp_ep2rp_bam_mixsizepkts_perf_seq");
    super.new(name);
  endfunction : new

  virtual function void build_phase(uvm_phase phase);
   `uvm_info ("build_phase", "[sysrp_ep2rp_bam_mixsizepkts_perf_seq] Entered Sequence Build Phase...",UVM_LOW);
   `uvm_info ("build_phase", "[sysrp_ep2rp_bam_mixsizepkts_perf_seq] Exiting Sequence Build Phase...",UVM_LOW)
  ...
  task body();
	...
	if(sel0for2G_sel1for6G == 0) begin //{
           `uvm_do_on_with(mem_request_seq,p_sequencer.endpoint_vseqr.endpoint_virt_seqr.driver_transaction_seqr[0],{ mem_request_seq.transaction_type == svt_pcie_driver_app_transaction::MEM_WR;
								      ...
								      mem_request_seq.length dist {16:=1, 32:=1, 48:=1, 64:=1};
