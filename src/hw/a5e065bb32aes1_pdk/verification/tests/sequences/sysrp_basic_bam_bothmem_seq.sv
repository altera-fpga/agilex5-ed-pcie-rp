class sysrp_basic_bam_bothmem_seq extends sysrp_base_seq;
  
  //rand int no_of_transactions ;
  rand int sequence_length ;

  `uvm_object_utils(sysrp_basic_bam_bothmem_seq);

  /** Declare a typed sequencer object that the sequence can access */
  `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

  function new (string name = "sysrp_basic_bam_bothmem_seq");
    super.new(name);
  endfunction : new

  virtual function void build_phase(uvm_phase phase);
   `uvm_info ("build_phase", "[sysrp_basic_bam_bothmem_seq] Entered Sequence Build Phase...",UVM_LOW);
   `uvm_info ("build_phase", "[sysrp_basic_bam_bothmem_seq] Exiting Sequence Build Phase...",UVM_LOW)
  endfunction: build_phase

  task body();
    bit[63:0] local_addr[$], l_addr;
    bit[9:0] local_len[$], l_len;
    ...
    super.body();
     ...
     m_b2b_mwr_enb = $urandom_range(0, 1);
     `uvm_info(get_name(), $psprintf("ST1: m_b2b_mwr_enb = %0d", m_b2b_mwr_enb), UVM_LOW)


     //1. m_b2b_mwr_enb =1
     if(m_b2b_mwr_enb == 1) begin //{ back-to-back MWr
        for (int i=0; i<sequence_length; i++) begin //{  //MWr
	   ...
	   #20ns;
           `uvm_info(get_name(), $psprintf("Transaction=%0d, addr=%0h, len=%0d, first_dw_be=%0h, last_dw_be=%0h", i, local_addr[i], local_len[i], local_first_dw_be[i], local_last_dw_be[i]), UVM_LOW)
        end //}
        #1us;

        for (int j=0; j<sequence_length; j++) begin //{ MRd
     ...
     #15us; //allow time for packets to go out
