class sysrp_ep2rp_bam_expandedmem_seq extends sysrp_base_seq;
  
  //rand int no_of_transactions ;
  rand int sequence_length ;

  `uvm_object_utils(sysrp_ep2rp_bam_expandedmem_seq);

  /** Declare a typed sequencer object that the sequence can access */
  `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

  function new (string name = "sysrp_ep2rp_bam_expandedmem_seq");
    super.new(name);
  endfunction : new

  virtual function void build_phase(uvm_phase phase);
   `uvm_info ("build_phase", "[sysrp_ep2rp_bam_expandedmem_seq] Entered Sequence Build Phase...",UVM_LOW);
   `uvm_info ("build_phase", "[sysrp_ep2rp_bam_expandedmem_seq] Exiting Sequence Build Phase...",UVM_LOW)
  ...
  task body();
    ...
    super.body();
     ...
     for (int i=0; i<sequence_length; i++) begin //{ 
        ...
        `uvm_info(get_name(), $psprintf("Transaction number=%0d, addr=%0h, len=%0d, first_dw_be=%0h, last_dw_be=%0h", i, local_addr, local_len, local_first_dw_be, local_last_dw_be), UVM_LOW)
