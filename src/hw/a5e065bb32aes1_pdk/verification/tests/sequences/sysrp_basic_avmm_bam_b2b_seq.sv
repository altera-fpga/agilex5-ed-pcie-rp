class sysrp_basic_avmm_bam_b2b_seq extends sysrp_base_seq;
  
   //rand int no_of_transactions ;
   rand int sequence_length ;
   rand int trans_no;

   `uvm_object_utils(sysrp_basic_avmm_bam_b2b_seq);

   /** Declare a typed sequencer object that the sequence can access */
   `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

   function new (string name = "sysrp_basic_avmm_bam_b2b_seq");
      super.new(name);
    endfunction : new

   virtual function void build_phase(uvm_phase phase);
      `uvm_info ("build_phase", "[sysrp_basic_avmm_bam_b2b_seq] Entered Sequence Build Phase...",UVM_LOW);
      `uvm_info ("build_phase", "[sysrp_basic_avmm_bam_b2b_seq] Exiting Sequence Build Phase...",UVM_LOW)
   ...
   task body();
     ...
     //-----------------------------------------------------------------------------
     //Perform Back to Back Memory Write/Read Transactions on F2H AVMM BAM interface from Endpoint
     //-----------------------------------------------------------------------------
     ...
     for (int j=0;j<3;j++) begin //{
        ...
        for (int i=0;i<trans_no;i++) begin //{
           ...
           `uvm_info(get_name(), $psprintf("Loop=%0d, write=%0d, addr=%0h, len=%0d, first_dw_be=%0h, last_dw_be=%0h",j , i, local_addr[i], local_len[i], local_first_dw_be[i], local_last_dw_be[i]), UVM_LOW)
        end //}

        //#200ns;
   
        for (int i=0;i<trans_no;i++) begin //{
           `uvm_info(get_name(), $psprintf("Loop=%0d, read=%0d, addr=%0h, len=%0d, first_dw_be=%0h, last_dw_be=%0h", j, i, local_addr[i], local_len[i], local_first_dw_be[i], local_last_dw_be[i]), UVM_LOW)
           `uvm_do_on_with(mem_request_seq,p_sequencer.endpoint_vseqr.endpoint_virt_seqr.driver_transaction_seqr[0],{ mem_request_seq.transaction_type == svt_pcie_driver_app_transaction::MEM_RD;
                                                                                  ...
                                                                                  mem_request_seq.ep == 0;
                                                                                  mem_request_seq.th == 0;
                                                                                  mem_request_seq.first_dw_be == local_first_dw_be[i];
                                                                                  mem_request_seq.last_dw_be == local_last_dw_be[i];
                                                                                  mem_request_seq.block == 1; }); // Must block to avoid r/w collision
