class random_uniq_address_from_hps;
    randc bit [31:0] address;
    constraint valid_address {address[31:12] inside {['h1_0000:'h1_ffff]}; address[11:0] == 0;} 
endclass


class sysrp_rp2ep_bas_perf_seq extends sysrp_base_seq;
    
  //rand int no_of_transactions ;
  rand int sequence_length ;
  rand int trans_no;

  `uvm_object_utils(sysrp_rp2ep_bas_perf_seq);

  /** Declare a typed sequencer object that the sequence can access */
  `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

  function new (string name = "sysrp_rp2ep_bas_perf_seq");
     super.new(name);
  endfunction : new

  virtual function void build_phase(uvm_phase phase);
     `uvm_info ("build_phase", "[sysrp_rp2ep_bas_perf_seq] Entered Sequence Build Phase...",UVM_LOW);
     `uvm_info ("build_phase", "[sysrp_rp2ep_bas_perf_seq] Exiting Sequence Build Phase...",UVM_LOW)
  ...
  task body();
     ...
     random_uniq_address_from_hps addr_obj;
