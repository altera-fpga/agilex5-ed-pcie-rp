class sysrp_bas_mps_mrrs_split_seq extends sysrp_base_seq;
    
  //rand int no_of_transactions ;
  rand int sequence_length ;
  rand int trans_no;

  `uvm_object_utils(sysrp_bas_mps_mrrs_split_seq);

  /** Declare a typed sequencer object that the sequence can access */
  `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

  function new (string name = "sysrp_bas_mps_mrrs_split_seq");
     super.new(name);
  endfunction : new

  virtual function void build_phase(uvm_phase phase);
     `uvm_info ("build_phase", "[sysrp_bas_mps_mrrs_split_seq] Entered Sequence Build Phase...",UVM_LOW);
     `uvm_info ("build_phase", "[sysrp_bas_mps_mrrs_split_seq] Exiting Sequence Build Phase...",UVM_LOW)
