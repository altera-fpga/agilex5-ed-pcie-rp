class sysrp_ep2rp_bam_midpkts_mrd_perf_seq extends sysrp_base_seq;
  
  //rand int no_of_transactions ;
  rand int sequence_length ;

  `uvm_object_utils(sysrp_ep2rp_bam_midpkts_mrd_perf_seq);

  /** Declare a typed sequencer object that the sequence can access */
  `uvm_declare_p_sequencer(sysrp_virtual_sequencer)

  function new (string name = "sysrp_ep2rp_bam_midpkts_mrd_perf_seq");
    super.new(name);
  endfunction : new

  virtual function void build_phase(uvm_phase phase);
   `uvm_info ("build_phase", "[sysrp_ep2rp_bam_midpkts_mrd_perf_seq] Entered Sequence Build Phase...",UVM_LOW);
   `uvm_info ("build_phase", "[sysrp_ep2rp_bam_midpkts_mrd_perf_seq] Exiting Sequence Build Phase...",UVM_LOW)
    ...
    super.body();
     ...
     for (int i=0; i<10; i++) begin //{ 
        `uvm_do_on_with(mem_request_seq,p_sequencer.endpoint_vseqr.endpoint_virt_seqr.driver_transaction_seqr[0],{ mem_request_seq.transaction_type == svt_pcie_driver_app_transaction::MEM_WR;
								      mem_request_seq.address[35:12] dist {[24'h08_0000 : 24'h0A_0000]:= 1,[24'h0A_0000 : 24'h0F_0000]:= 1, [24'h88_0000 : 24'h90_0000]:= 1, [24'h90_0000 : 24'h9F_0000]:= 1};
     ...
     #4us; //delay between MWr and MRd 
     for (int j=0; j <(sequence_length/10); j++) begin //{
	for (int k=0; k <10; k++) begin //{
           `uvm_info(get_name(), $psprintf("Transaction number=%0d, addr=%0h, len=%0d, first_dw_be=%0h, last_dw_be=%0h", k, local_addr[k], local_len[k], local_first_dw_be[k], local_last_dw_be[k]), UVM_LOW)
           ...
           `uvm_do_on_with(mem_request_seq,p_sequencer.endpoint_vseqr.endpoint_virt_seqr.driver_transaction_seqr[0],{ mem_request_seq.transaction_type == svt_pcie_driver_app_transaction::MEM_RD;
                                                                      mem_request_seq.address == local_addr[k];
                                                                      mem_request_seq.length == local_len[k];
                                                                      mem_request_seq.traffic_class == 0;
                                                                      mem_request_seq.address_translation == 0;
                                                                      mem_request_seq.ep == 0;
                                                                      mem_request_seq.th == 0;
                                                                      mem_request_seq.first_dw_be == local_first_dw_be[k];
                                                                      mem_request_seq.last_dw_be == local_last_dw_be[k];
                                                                      mem_request_seq.block == 0; }); // Must block to avoid r/w collision
	end //}
     end //}
