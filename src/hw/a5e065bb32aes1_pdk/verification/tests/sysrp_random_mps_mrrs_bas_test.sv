/**
 * Abstract: sysrp_random_mps_mrrs_bas_test 
   Perform transactions from HPS to EP with different configurations 
   for MPS and MRRS done randomly from testcase.
* 
 */
class randomize_mps_mrrs;
  randc int mps,mrrs;
  constraint check      { mps >= mrrs;}
  constraint mps_valid  { mps inside  {128,256,512};}
  constraint mrrs_valid { mrrs inside {128,256,512};}
endclass;


 class sysrp_random_mps_mrrs_bas_test extends sysrp_base_test;
	int RP_MPS_value ;
	int EP_MPS_value ;
  int MRRS_value ;
  int pcie_tx_scbd;
  ...
  `uvm_component_utils(sysrp_random_mps_mrrs_bas_test)

  /** Class Constructor */
  function new(string name = "sysrp_random_mps_mrrs_bas_test", uvm_component parent=null);
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    int max_rd_req;
    randomize_mps_mrrs obj;
    pcie_tx_scbd = 0;
    axi_tx_scbd  = 1;
    obj=new();
    obj.randomize();
   `uvm_info ("build_phase", "[sysrp_random_mps_mrrs_bas_test] Entered Build Phase...",UVM_LOW);

       super.build_phase(phase);
       ep_max_payload_size = obj.mps;
       rp_max_payload_size = obj.mps;
       max_rd_req          = obj.mrrs;
   ...
   `uvm_info ("build_phase", "[sysrp_random_mps_mrrs_bas_test] Exiting Build Phase...",UVM_LOW)
