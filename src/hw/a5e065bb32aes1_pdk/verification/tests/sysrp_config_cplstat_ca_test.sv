/**
 * Abstract:  sysrp_config_cplstat_ca_test
...
///--- class cs_cplca_pcie_target_callback  ---
class cs_cplca_pcie_target_callback extends svt_pcie_target_app_callback;
   svt_pcie_tlp_exception my_tlp_exc = new("my_tlp_exc");

   `uvm_object_utils(cs_cplca_pcie_target_callback)

   function new(string name ="cs_cplca_pcie_target_callback");
      super.new(name);
   endfunction
   ...
   virtual function void pre_tx_tlp_put(svt_pcie_target_app target_app, svt_pcie_tlp transaction, ref bit drop);
	...
	transaction.completion_status = svt_pcie_tlp::COMPLETER_ABORT;
     end
...
endclass: cs_cplca_pcie_target_callback

///---class sysrp_config_cplstat_ca_test ---
class sysrp_config_cplstat_ca_test extends sysrp_base_test;
   int pcie_tx_scbd;
   int axi_tx_scbd;

   /** UVM Component Utility macro */
   `uvm_component_utils(sysrp_config_cplstat_ca_test)

   cs_cplca_pcie_target_callback cs_cpl_ep_packet;

   /** Class Constructor */
   function new(string name = "sysrp_config_cplstat_ca_test", uvm_component parent=null);
  ...
  virtual function void build_phase(uvm_phase phase);
     ...
     `uvm_info ("build_phase", "[sysrp_config_cplstat_ca_test] Entered Build Phase...",UVM_LOW);
     ...
     `uvm_info ("build_phase", "[sysrp_config_cplstat_ca_test] Exiting Build Phase...",UVM_LOW)
  ...
  task run_phase(uvm_phase phase);
    ...
    `uvm_info("INFO", $sformatf("Running traffic sequence PART1"),UVM_LOW);
     ...
     `uvm_info("INFO", $sformatf("Call back Cpl/CplD with UR/CA PART2"),UVM_LOW);
