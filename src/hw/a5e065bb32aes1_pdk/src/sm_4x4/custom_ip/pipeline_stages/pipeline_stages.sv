...
input   logic [BURSTCNT_WIDTH-1:0]    s0_avmm_burstcnt,
...
output logic [BURSTCNT_WIDTH-1:0]    m0_avmm_burstcnt,
...
input  logic                         m0_avmm_waitrequest
 ...
 reg [PIPE_STAGES-1:0][BURSTCNT_WIDTH-1:0]     s0_pipe_avmm_burstcnt='d0;
 ...
 always@(posedge clk) begin
  if(!rst_n) begin
    for(i=0; i<PIPE_STAGES; i++) begin
     ...
     s0_pipe_avmm_burstcnt         <= {s0_pipe_avmm_burstcnt[PIPE_STAGES-1:0],s0_avmm_burstcnt};
...
assign m0_avmm_burstcnt       = s0_pipe_avmm_burstcnt     [PIPE_STAGES-1];
